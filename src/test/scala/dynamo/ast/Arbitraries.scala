...
object Arbitraries {
  ...
  def kvGen: Gen[(String, DynamoType)] =
    for {
      key   <- identifier
      value <- DynamoTypeGen
    } yield (key, value)
  ...
  implicit val DynamoListReadSuccessfulArb: Arbitrary[List[DynamoRead[DynamoType]]] = Arbitrary[List[DynamoRead[DynamoType]]] {
    listOfN(size, DynamoReadSuccessfulGen)
  }
